#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
╔══════════════════════════════════════════════════════════╗
║          بوت تداول USDT P2P المتكامل - النسخة النهائية    ║
║   ✅ شراء/بيع | ✅ عروض | ✅ إحالات | ✅ إدارة مخزون       ║
║   🚀 نظام إرسال السكرين شوت للإدارة (Fixed)              ║
╚══════════════════════════════════════════════════════════╝
"""

import os, sqlite3, logging, random, string, threading
from datetime import datetime
from threading import local
from telegram import (
    Update, InlineKeyboardButton, InlineKeyboardMarkup,
    ReplyKeyboardMarkup, KeyboardButton, ReplyKeyboardRemove
)
from telegram.ext import (
    Application, CommandHandler, MessageHandler, CallbackQueryHandler,
    ConversationHandler, filters, ContextTypes
)
from telegram.constants import ParseMode

# ════════════════════════════════════════════════════════════
#                    ⚙️  الإعدادات الأساسية
# ════════════════════════════════════════════════════════════
BOT_TOKEN       = "8443614197:AAFF5awBt6UX3ZAcxsosWuDkVUUq8GOmuRg"
OWNER_ID        = 6814152338
DEPOSIT_ADDRESS = "TYourTRC20AddressHere" # ضع محفظتك الحقيقية هنا
DB_PATH         = "trading_bot.db"

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# ════════════════════════════════════════════════════════════
#                    🗄️  قاعدة البيانات
# ════════════════════════════════════════════════════════════
_db_local = local()
def get_conn():
    if not hasattr(_db_local, 'conn') or _db_local.conn is None:
        c = sqlite3.connect(DB_PATH, check_same_thread=False, isolation_level=None)
        c.row_factory = sqlite3.Row
        _db_local.conn = c
    return _db_local.conn

def init_db():
    conn = get_conn()
    conn.executescript('''
        CREATE TABLE IF NOT EXISTS users (
            user_id INTEGER PRIMARY KEY, username TEXT, full_name TEXT, country TEXT, 
            balance_usdt REAL DEFAULT 0, commission_earned REAL DEFAULT 0, rank TEXT DEFAULT 'user',
            referral_code TEXT UNIQUE, referred_by INTEGER, is_banned INTEGER DEFAULT 0,
            created_at TEXT DEFAULT CURRENT_TIMESTAMP
        );
        CREATE TABLE IF NOT EXISTS orders (
            order_id INTEGER PRIMARY KEY AUTOINCREMENT, seller_id INTEGER, buyer_id INTEGER,
            order_type TEXT, amount_usdt REAL, price_per_usdt REAL, total_sdg REAL, 
            status TEXT DEFAULT 'pending', payment_proof TEXT, user_account_details TEXT
        );
        CREATE TABLE IF NOT EXISTS payment_methods (method_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, details TEXT);
        CREATE TABLE IF NOT EXISTS receive_methods (method_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, details TEXT);
        CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT);
        
        INSERT OR IGNORE INTO settings VALUES ('usdt_buy_price', '4000.0'), ('usdt_sell_price', '4100.0'), 
        ('trader_online_status', '1'), ('available_usdt_inventory', '1000.0');
    ''')

def db_get(q, p=()): return dict(r) if (r := get_conn().execute(q, p).fetchone()) else None
def db_all(q, p=()): return [dict(r) for r in get_conn().execute(q, p).fetchall()]
def db_run(q, p=()): return get_conn().execute(q, p).lastrowid
def get_s(k): r = db_get("SELECT value FROM settings WHERE key=?", (k,)); return r['value'] if r else '0'
def set_s(k, v): db_run("INSERT OR REPLACE INTO settings VALUES (?,?)", (k, str(v)))
def is_admin(uid): return uid == OWNER_ID
def fmt(n): return f"{float(n):,.2f}"

# ════════════════════════════════════════════════════════════
#                    🔢  حالات المحادثة
# ════════════════════════════════════════════════════════════
(REG_NAME, REG_COUNTRY, BUY_AMT, BUY_CONF, BUY_PAY_SEL, BUY_PROOF, BUY_ACC, BUY_DONE, 
 SELL_AMT, SELL_CONF, SELL_PROOF, SELL_ACC, SELL_DONE, OFFER_TYPE, OFFER_AMT, OFFER_PRICE, OFFER_CONF) = range(17)

# ════════════════════════════════════════════════════════════
#                    ⌨️  لوحات المفاتيح
# ════════════════════════════════════════════════════════════
def kb_main(admin=False):
    rows = [
        [KeyboardButton("🛒 شراء USDT"), KeyboardButton("💰 بيع USDT")],
        [KeyboardButton("💵 تقديم عرض"), KeyboardButton("👤 ملفي")],
        [KeyboardButton("💳 رصيدي"), KeyboardButton("📋 طلباتي")],
        [KeyboardButton("💱 سعر الصرف"), KeyboardButton("🗃️ المخزون")],
        [KeyboardButton("📞 الدعم"), KeyboardButton("🔗 الإحالة")]
    ]
    if admin: rows.append([KeyboardButton("🔐 الإدارة")])
    return ReplyKeyboardMarkup(rows, resize_keyboard=True)

def kb_admin():
    return ReplyKeyboardMarkup([
        [KeyboardButton("📋 جميع الطلبات"), KeyboardButton("🚦 حالة التاجر")],
        [KeyboardButton("💲 تعديل الأسعار"), KeyboardButton("🔙 الرئيسية")]
    ], resize_keyboard=True)

def kb_cancel(): return ReplyKeyboardMarkup([[KeyboardButton("❌ إلغاء")]], resize_keyboard=True)
def kb_confirm(): return ReplyKeyboardMarkup([[KeyboardButton("✅ تأكيد"), KeyboardButton("❌ إلغاء")]], resize_keyboard=True)

# ════════════════════════════════════════════════════════════
#                    🚀 منطق البوت الأساسي
# ════════════════════════════════════════════════════════════

async def start(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    uid = upd.effective_user.id
    if not db_get("SELECT * FROM users WHERE user_id=?", (uid,)):
        await upd.message.reply_text("👋 مرحباً بك! يرجى إدخال اسمك الكامل للتسجيل:", reply_markup=kb_cancel())
        return REG_NAME
    await upd.message.reply_text("مرحباً بك مجدداً في المنصة!", reply_markup=kb_main(is_admin(uid)))
    return ConversationHandler.END

async def reg_name(upd, ctx):
    ctx.user_data['name'] = upd.message.text
    await upd.message.reply_text("🌍 أدخل اسم بلدك:", reply_markup=kb_cancel())
    return REG_COUNTRY

async def reg_country(upd, ctx):
    uid = upd.effective_user.id
    ref = ''.join(random.choices(string.ascii_uppercase + string.digits, k=8))
    db_run("INSERT INTO users (user_id, full_name, country, referral_code) VALUES (?,?,?,?)",
           (uid, ctx.user_data['name'], upd.message.text, ref))
    await upd.message.reply_text("✅ تم التسجيل بنجاح!", reply_markup=kb_main(is_admin(uid)))
    return ConversationHandler.END

# 🛒 نظام شراء USDT
async def buy_start(upd, ctx):
    if get_s('trader_online_status') == '0' and not is_admin(upd.effective_user.id):
        await upd.message.reply_text("⛔️ المتجر مغلق حالياً.")
        return ConversationHandler.END
    await upd.message.reply_text("🛒 أدخل كمية USDT المراد شراؤها:", reply_markup=kb_cancel())
    return BUY_AMT

async def buy_amt(upd, ctx):
    try:
        amt = float(upd.message.text)
        ctx.user_data['amt'] = amt
        ctx.user_data['total'] = amt * float(get_s('usdt_sell_price'))
        await upd.message.reply_text(f"💰 الإجمالي: {fmt(ctx.user_data['total'])} SDG\nهل تود المتابعة؟", reply_markup=kb_confirm())
        return BUY_CONF
    except: return BUY_AMT

async def buy_conf(upd, ctx):
    if upd.message.text == "✅ تأكيد":
        methods = db_all("SELECT * FROM payment_methods")
        if not methods: 
            await upd.message.reply_text("لا توجد طرق دفع حالياً.")
            return ConversationHandler.END
        btns = [[InlineKeyboardButton(m['name'], callback_data=f"bpay_{m['method_id']}")] for m in methods]
        await upd.message.reply_text("💳 اختر وسيلة الدفع:", reply_markup=InlineKeyboardMarkup(btns))
        return BUY_PAY_SEL
    return await cancel(upd, ctx)

async def buy_pay_sel(upd, ctx):
    q = upd.callback_query
    await q.answer()
    mid = q.data.split("_")[1]
    m = db_get("SELECT * FROM payment_methods WHERE method_id=?", (mid,))
    await q.edit_message_text(f"🔢 حول للمبلغ إلى:\n`{m['details']}`\n\n📸 أرسل صورة الإيصال (Screenshot) الآن:")
    return BUY_PROOF

async def buy_proof(upd, ctx):
    if not upd.message.photo:
        await upd.message.reply_text("أرسل صورة الإيصال فقط!")
        return BUY_PROOF
    ctx.user_data['proof'] = upd.message.photo[-1].file_id
    await upd.message.reply_text("📝 أدخل عنوان محفظتك لاستلام الـ USDT:")
    return BUY_ACC

async def buy_done(upd, ctx):
    uid = upd.effective_user.id
    oid = db_run("INSERT INTO orders (buyer_id, order_type, amount_usdt, total_sdg, status, payment_proof, user_account_details) VALUES (?,?,?,?,?,?,?)",
           (uid, 'buy', ctx.user_data['amt'], ctx.user_data['total'], 'pending', ctx.user_data['proof'], upd.message.text))
    
    # إرسال للإدارة فوراً
    await ctx.bot.send_photo(chat_id=OWNER_ID, photo=ctx.user_data['proof'], 
                             caption=f"🔔 طلب شراء جديد #{oid}\nالمبلغ: {fmt(ctx.user_data['total'])} SDG\nالمحفظة: {upd.message.text}")
    
    await upd.message.reply_text("🚀 تم إرسال طلبك! سيراجع المسؤول الإيصال ويحول لك.", reply_markup=kb_main(is_admin(uid)))
    return ConversationHandler.END

# 🛠 وظائف عامة
async def show_exchange(upd, ctx):
    buy = get_s('usdt_buy_price')
    sell = get_s('usdt_sell_price')
    await upd.message.reply_text(f"💱 أسعار الصرف:\n📥 شراء: {buy} SDG\n📤 بيع: {sell} SDG")

async def show_inventory(upd, ctx):
    inv = get_s('available_usdt_inventory')
    await upd.message.reply_text(f"🗃️ المخزون المتاح: {inv} USDT")

async def cancel(upd, ctx):
    await upd.message.reply_text("تم الإلغاء.", reply_markup=kb_main(is_admin(upd.effective_user.id)))
    return ConversationHandler.END

# 🔐 الإدارة
async def admin_orders(upd, ctx):
    if not is_admin(upd.effective_user.id): return
    orders = db_all("SELECT * FROM orders WHERE status='pending' LIMIT 5")
    if not orders:
        await upd.message.reply_text("لا توجد طلبات معلقة.")
        return
    for o in orders:
        txt = f"📦 طلب #{o['order_id']} | النوع: {o['order_type']}\nالمبلغ: {o['amount_usdt']} USDT"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅ قبول", callback_data=f"acc_{o['order_id']}"), 
                                    InlineKeyboardButton("❌ رفض", callback_data=f"rej_{o['order_id']}")]])
        if o['payment_proof']: await upd.message.reply_photo(o['payment_proof'], caption=txt, reply_markup=kb)
        else: await upd.message.reply_text(txt, reply_markup=kb)

# 🏁 تشغيل
def main():
    init_db()
    app = Application.builder().token(BOT_TOKEN).build()
    
    # تسجيل الحوارات (Conversations)
    buy_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^🛒 شراء USDT$"), buy_start)],
        states={
            BUY_AMT: [MessageHandler(filters.TEXT & ~filters.COMMAND, buy_amt)],
            BUY_CONF: [MessageHandler(filters.TEXT & ~filters.COMMAND, buy_conf)],
            BUY_PAY_SEL: [CallbackQueryHandler(buy_pay_sel, pattern="^bpay_")],
            BUY_PROOF: [MessageHandler(filters.PHOTO, buy_proof)],
            BUY_ACC: [MessageHandler(filters.TEXT & ~filters.COMMAND, buy_done)],
        }, fallbacks=[MessageHandler(filters.Regex("^❌ إلغاء$"), cancel)]
    )
    
    reg_conv = ConversationHandler(
        entry_points=[CommandHandler("start", start)],
        states={
            REG_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, reg_name)],
            REG_COUNTRY: [MessageHandler(filters.TEXT & ~filters.COMMAND, reg_country)],
        }, fallbacks=[MessageHandler(filters.Regex("^❌ إلغاء$"), cancel)]
    )

    app.add_handler(buy_conv)
    app.add_handler(reg_conv)
    app.add_handler(MessageHandler(filters.Regex("^💱 سعر الصرف$"), show_exchange))
    app.add_handler(MessageHandler(filters.Regex("^🗃️ المخزون$"), show_inventory))
    app.add_handler(MessageHandler(filters.Regex("^🔐 الإدارة$"), lambda u,c: u.message.reply_text("لوحة الإدارة:", reply_markup=kb_admin())))
    app.add_handler(MessageHandler(filters.Regex("^📋 جميع الطلبات$"), admin_orders))
    app.add_handler(MessageHandler(filters.Regex("^🔙 الرئيسية$"), lambda u,c: u.message.reply_text("القائمة الرئيسية:", reply_markup=kb_main(is_admin(u.effective_user.id)))))

    print("🚀 البوت يعمل الآن...")
    app.run_polling()

if __name__ == "__main__":
    main()
